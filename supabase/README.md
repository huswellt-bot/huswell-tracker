...
5. Run `004_project_manager_customer_access.sql` to allow Sales Executives to create and manage customers.
...
17. Run `016_project_officer_private_submissions.sql` to keep Sales Executive costings/quotations private and send submitted work to Owner / General Manager.
...
24. Run `023_project_manager_caller_directory.sql` so the Outbound Caller dropdown can list your Sales Executives.
...
35. Run `036_general_manager_review_and_project_edits.sql` for General Manager review refinements and Sales Executive project-edit approvals.
...
40. Run `041_sales_project_officer_signatures.sql` before using Sales Executive signatures in PDFs.
41. Run `042_auto_assign_lead_creator.sql` and `043_restrict_lead_reads_to_creators.sql` to keep new Lead ownership aligned with the creating Sales Executive.
42. Run `044_sales_project_officer_lead_change_approvals.sql` to require General Manager approval for Sales Executive Lead edits and deletions.
...
46. Run `048_shared_open_costings_kpi.sql` to include the organization-wide Open Costings total on the Sales Executive KPI cards.
...
50. Run `052_project_officer_kpi_aggregates.sql` to show organization and personal KPI counts for each Sales Executive. Run this before deploying the corresponding app code.
...
60. Run `062_chatbot_lead_intake_and_assignment.sql` before deploying chatbot lead intake. It adds idempotent chatbot lead receipt, lets the General Manager assign a lead to a Sales Executive, and limits each officer to their assigned leads.
...
64. Run `065_price_quotation_revision_approvals.sql` to require General Manager approval before a Sales Executive can revise an approved direct Price Quotation.
65. Run `066_remove_price_quotation_expiry.sql` to remove Price Quotation expiry dates and the default Validity term.
66. Run `067_fix_price_quotation_draft_totals_guard.sql` to let a Sales Executive save item-only revisions without bypassing General Manager price protection.
67. Run `068_exclusive_project_due_dates.sql` before migration 070 to install the initial project due-date integrity guard.
68. Run `069_price_quotation_project_type.sql` to let Sales Executives save Project Type on direct Price Quotations.
...
71. Run `072_project_schedule_progress.sql` to add protected Sales Executive progress percentage and remark updates.
...
76. Run `077_officer_unsubmit_pending_requests.sql` to let Sales Executives withdraw their own pending edit, revision, and completion requests before General Manager review.
...
94. Run `095_allow_price_quotation_item_content_revisions.sql` to let Sales Executives revise direct quotation content without changing General Manager prices.
...
99. Historical migration `104_price_quotation_endorsements.sql` introduced the quotation-PDF endorsement feature; migration `168_commission_summary_and_lead_endorsement_lock.sql` permanently removes that feature and its data after the required Storage API cleanup.
...
102. In Super Admin user management, create or edit the two Sales & Pricing Officer accounts and assign each officer the project types they should review, including `Large Format Printing` where applicable. A quotation cannot be submitted until its project type has an assignment.
103. Run `117_mockup_quotation_workflow.sql` after `116_disable_standalone_pricing_officer.sql`, then deploy the matching app update. It adds the approved Price Quotation -> Mockup Quotation flow, independent mockup costing and General Manager approval, the professional Quotation Costing Overview, Legal-size Mockup Quotation PDFs, and private signed-client-proof attachments (up to five images per approved quotation). The previous Mockups screen is replaced, but the legacy mockup rows and storage objects are intentionally retained until authenticated role testing and an organization-scoped backup/preflight are complete.
104. Run `118_sales_pricing_officer_price_review_guard.sql` after `117_mockup_quotation_workflow.sql`, then run `119_sales_pricing_officer_quotation_totals_guard.sql`, `120_sales_pricing_officer_self_review.sql`, and `121_general_manager_price_quotation_edits.sql` before deploying the matching app update. These migrations let the assigned Sales & Pricing Officer save calculated direct Price Quotation selling prices during pending review, including quotations they prepared, and submit them to the General Manager, while Sales Executives remain blocked from setting Selling Price / Unit or quotation totals. Migration `121` lets the General Manager persist edited costings, VAT, terms, and bank details when approving a direct Price Quotation.
105. Run `122_production_request_workflow.sql` before deploying the production-request UI. It adds the approved Mockup Quotation and two selected signed-proof references to Project Calendar requests, links approved requests to the existing Production Job, and permits Production/Warehouse users to view those linked private proof images.
106. Run `123_quotation_payment_tracking.sql` before deploying the payment-tracking UI. It adds payment terms and a separate receipt-backed payment ledger for approved direct Price Quotations, allows Sales Executives and Sales & Pricing Officers to submit receipts, and provides the initial General Manager-only verification path. Payment status is visible in quotation and Production views; this migration does not automatically block production or delivery on payment status.
107. Run `124_quotation_payment_reviewer_routing.sql` after `123_quotation_payment_tracking.sql` before deploying the management Approval Center. It assigns pending receipt reviews to the Sales & Pricing Officer who priced the quotation, keeps General Manager override access, prevents a receipt submitter from verifying their own receipt, and records the reviewer on each payment row.
108. Run `125_enforce_quotation_payment_amounts.sql` after `124_quotation_payment_reviewer_routing.sql` before deploying the updated payment form. It enforces downpayment targets, installment limits, exact final-payment amounts, and prevents new payment submissions from exceeding the available balance.
109. Run `126_verified_receipt_sales_commissions.sql`, then `127_separate_mockup_requirements.sql`, before deploying the matching Mockup Quotation update. Migration `127` keeps the approved Price Quotation link and inherited client/project details, but makes Mockup descriptions and quantities independent requirements rather than copied source items.
110. Run `128_mockup_quotation_workflow_parity.sql` after `127_separate_mockup_requirements.sql`. It restores the Mockup request, review, revision, and General Manager approval transitions after the direct Price Quotation workflow guard, and allows General Manager costing, VAT, terms, and bank-detail edits before Mockup approval. Endorsements remain Price Quotation-only.
111. Run `129_mockup_quotation_preparation_parity.sql` after `128_mockup_quotation_workflow_parity.sql` before deploying the matching app update. It gives Mockup Quotations the same draft, submit, withdraw, and approved-revision controls as direct Price Quotations, while retaining their source link and independent requirements/costing.
112. Run `130_mockup_quotation_payment_tracking.sql` after `129_mockup_quotation_preparation_parity.sql` before deploying the matching app update. It gives approved Mockup Quotations their own payment terms, receipt ledger, amount enforcement, assigned payment review, and receipt-storage access; existing Price Quotation payment records remain attached to their original quotation.
...
118. Run `136_pricing_markups_percentage_only.sql` after `135_vat_percentage_only.sql` and before deploying the matching pricing update. It makes new and edited internal pricing markups percentage-only while preserving historical fixed-amount quotation rows until they are reviewed again.
119. Run `137_direct_cost_markup_discount_and_review_vat.sql` after `136_pricing_markups_percentage_only.sql` and before deploying the matching pricing update. It removes Discount from General Manager defaults without changing saved quotation discounts; then calculates all internal markups, including a hidden 12% Internal VAT, from Total Direct Cost, applies the Pricing Officer discount and separate GM review VAT, and saves the grand-total-per-piece price. Only the GM review VAT appears on the customer PDF.
120. Run `138_gm_pricing_revision_routing.sql` after `137_direct_cost_markup_discount_and_review_vat.sql` and before deploying the matching quotation-review update. It records whether a revision is directed to the Project Officer or Pricing Officer, routes GM returns to the assigned Pricing Officer with the GM note, updates the quotation and item preparation guards for that revision path, discards unsaved GM edits, and allows the Pricing Officer to resubmit the revised Price or Mockup Quotation to GM review. If migration 138 was already run before this guard update, rerun the idempotent script before deploying the app.
...
128. Run `146_refresh_pending_gm_pricing_defaults.sql` after migration 145 and before deploying the matching workspace update. It refreshes the latest Settings-driven markups for target-margin quotations at GM review and final approval, preserves quotation Discount and VAT inputs, and leaves legacy markup quotations and approved quotations unchanged.
129. Run `147_lead_endorsements.sql` after migration 146 and before deploying the matching Leads workspace update. It lets a Sales Executive endorse an owned lead to one selected Sales & Pricing Officer, records the endorsing user and date, preserves the original owner, and grants visibility to the selected officer while General Managers retain full visibility.
130. Run `148_endorsed_lead_quotation_workflow.sql` after migration `147_lead_endorsements.sql` and before deploying the matching workspace update. It treats the selected Sales & Pricing Officer as an alternate assignee for lead actions, Price Quotations, Mockup Quotations, revisions, signed proofs, and production requests, while preserving the original owner, GM approval, and project-type routing controls.
131. Run `149_price_quotation_only_production.sql` after migration `148_endorsed_lead_quotation_workflow.sql` and before deploying the matching workspace update. It makes production requests require only an approved direct Price Quotation; Mockup Quotation and signed-proof columns, records, and storage objects remain nullable and preserved for legacy compatibility.
132. Run `150_pricing_submission_notes.sql` after migration 149 and before deploying the matching workspace update. It adds the optional Pricing Officer note to the Price Quotation GM-approval path while retaining the existing nine-argument RPC for older clients.
133. Run `151_lead_endorsement_approval.sql` after migration 148 and before deploying the matching workspace update. It adds permanent endorsement-history locking, original-owner unendorsement requests, endorsed-officer approval, and General Manager fallback approval without transferring lead ownership.
134. Before running `152_remove_legacy_quotation_proof_features.sql`, run `npm.cmd run cleanup:retired-quotation-storage` from the repository root with the server-only `SUPABASE_SERVICE_ROLE_KEY` available in `.env.local` or the shell. Use `npm.cmd run cleanup:retired-quotation-storage -- --dry-run` to inspect the two exact bucket targets first. The script removes their objects and buckets through the Supabase Storage API; then migration 152 performs the SQL metadata/security cleanup, clears legacy production references, disables the old RPCs, and leaves only empty legacy relations/nullable columns for backward compatibility. Never delete directly from `storage.objects` or `storage.buckets`.
135. Run `153_pricing_officer_direct_cost_only.sql` after migration 150 and before deploying the matching workspace update. It makes the Pricing Officer costing RPC ignore all submitted markup, discount, and pricing-model values, preserving saved internal pricing or applying organization defaults while accepting direct-cost lines only.
...
150. Before running `168_commission_summary_and_lead_endorsement_lock.sql`, export/backup the existing payment-based commission and quotation-PDF endorsement data if it must be retained for audit. Run `npm.cmd run cleanup:retired-quotation-storage -- --dry-run`, then `npm.cmd run cleanup:retired-quotation-storage` with the server-only Supabase service key. Migration 168 permanently removes the old payout/quotation-endorsement tables, RPCs, fields, and settings; adds GM/Finance Commission Summary rows for direct Price Quotations that are exactly `in_production`; snapshots Commission and VA Commission rates; adds the GM undo-paid audit path; and blocks lead endorsement removal or replacement after an approved Price Quotation. Apply the migration before deploying the matching workspace update.
151. Run `169_commission_summary_queued_eligibility.sql` after migration 168 and before deploying the matching Commission Summary update. It changes both the eligible-quotation picker and the server-side create guard so approved direct Price Quotations linked to a `queued` production job can receive a Commission Summary.
152. Run `170_commission_summary_calendar_eligibility.sql` after migration 169 and before deploying the matching Commission Summary update. It makes an approved, active Project Calendar schedule with a due date and an explicit production-job link the source of Commission Summary eligibility; orphan queued jobs no longer appear.
153. Run `171_automatic_commission_receivable_balance.sql` after migration 170 and before deploying the matching Commission Summary update. It derives Receivable / Due Balance from the saved Grand Total minus Downpayment Amount, rejects downpayments above the Grand Total, backfills existing summary balances, and keeps the existing RPC parameter only for rollout compatibility.
...
## Sales commissions from verified quotation receipts

Run `126_verified_receipt_sales_commissions.sql` after migrations `123` through `125` and before deploying the matching application code. It applies the VAT-exclusive tier rule (first PHP 300,000 at 3%, then the remainder at 5%), captures the final quotation workflow owner, accrues earnings from verified quotation receipts, and gives the General Manager controlled commission-payout recording.
