-- Add fixed-amount lines to General Manager-only Price Quotation costings.
-- Existing lines remain quantity × unit cost rows. Fixed expenses persist as
-- quantity 1 and their entered amount in unit_cost, with the type retained so
-- the UI and internal PDF show them as lump-sum expenses.
--
-- Run after 095_allow_price_quotation_item_content_revisions.sql. Safe to re-run.

begin;

alter table public.price_quotation_costing_lines
  add column if not exists calculation_type text not null default 'quantity_unit_cost';

alter table public.price_quotation_costing_lines
  drop constraint if exists price_quotation_costing_lines_calculation_type_check;
alter table public.price_quotation_costing_lines
  add constraint price_quotation_costing_lines_calculation_type_check
  check (calculation_type in ('quantity_unit_cost', 'fixed_amount'));
