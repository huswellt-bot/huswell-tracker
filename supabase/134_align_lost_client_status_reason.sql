-- Keep the status-3 approval guard aligned with the renamed Lost client label.
-- Status 3 is the legacy Dropped Client value, so this preserves existing
-- records and the requirement for a reason when an officer selects it.
-- Run after 133_pricing_officer_discount_markup.sql.
...
begin
  ...
  if p_change_type = 'update' then
    ...
    if (v_changes ->> 'evaluation_number') = '3'
      ...
      raise exception 'A lost client reason is required';
    end if;
  else
